require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe MocksController do

  # This should return the minimal set of attributes required to create a valid
  # Mock. As you add validations to Mock, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "creator_id" => "1" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # MocksController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all mocks as @mocks" do
      mock = Mock.create! valid_attributes
      get :index, {}, valid_session
      assigns(:mocks).should eq([mock])
    end
  end

  describe "GET show" do
    it "assigns the requested mock as @mock" do
      mock = Mock.create! valid_attributes
      get :show, {:id => mock.to_param}, valid_session
      assigns(:mock).should eq(mock)
    end
  end

  describe "GET new" do
    it "assigns a new mock as @mock" do
      get :new, {}, valid_session
      assigns(:mock).should be_a_new(Mock)
    end
  end

  describe "GET edit" do
    it "assigns the requested mock as @mock" do
      mock = Mock.create! valid_attributes
      get :edit, {:id => mock.to_param}, valid_session
      assigns(:mock).should eq(mock)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Mock" do
        expect {
          post :create, {:mock => valid_attributes}, valid_session
        }.to change(Mock, :count).by(1)
      end

      it "assigns a newly created mock as @mock" do
        post :create, {:mock => valid_attributes}, valid_session
        assigns(:mock).should be_a(Mock)
        assigns(:mock).should be_persisted
      end

      it "redirects to the created mock" do
        post :create, {:mock => valid_attributes}, valid_session
        response.should redirect_to(Mock.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved mock as @mock" do
        # Trigger the behavior that occurs when invalid params are submitted
        Mock.any_instance.stub(:save).and_return(false)
        post :create, {:mock => { "creator_id" => "invalid value" }}, valid_session
        assigns(:mock).should be_a_new(Mock)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Mock.any_instance.stub(:save).and_return(false)
        post :create, {:mock => { "creator_id" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested mock" do
        mock = Mock.create! valid_attributes
        # Assuming there are no other mocks in the database, this
        # specifies that the Mock created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Mock.any_instance.should_receive(:update).with({ "creator_id" => "1" })
        put :update, {:id => mock.to_param, :mock => { "creator_id" => "1" }}, valid_session
      end

      it "assigns the requested mock as @mock" do
        mock = Mock.create! valid_attributes
        put :update, {:id => mock.to_param, :mock => valid_attributes}, valid_session
        assigns(:mock).should eq(mock)
      end

      it "redirects to the mock" do
        mock = Mock.create! valid_attributes
        put :update, {:id => mock.to_param, :mock => valid_attributes}, valid_session
        response.should redirect_to(mock)
      end
    end

    describe "with invalid params" do
      it "assigns the mock as @mock" do
        mock = Mock.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Mock.any_instance.stub(:save).and_return(false)
        put :update, {:id => mock.to_param, :mock => { "creator_id" => "invalid value" }}, valid_session
        assigns(:mock).should eq(mock)
      end

      it "re-renders the 'edit' template" do
        mock = Mock.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Mock.any_instance.stub(:save).and_return(false)
        put :update, {:id => mock.to_param, :mock => { "creator_id" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested mock" do
      mock = Mock.create! valid_attributes
      expect {
        delete :destroy, {:id => mock.to_param}, valid_session
      }.to change(Mock, :count).by(-1)
    end

    it "redirects to the mocks list" do
      mock = Mock.create! valid_attributes
      delete :destroy, {:id => mock.to_param}, valid_session
      response.should redirect_to(mocks_url)
    end
  end

end
